...
def clinic_send(request, code):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            token = make_verify_token(code, parent_phone)
...
def verify_phone(request, code, token):
    """
    1) Decode the token to get expected last-10 digits & professional code.
    ...
    try:
        data = read_verify_token(token, max_age_days=7)  # default 7 days validity
        expected_last10 = data.get("p", "")
        token_code = data.get("c")
        if token_code != code:
            raise BadSignature("Code mismatch")
    except (BadSignature, SignatureExpired):
        ctx = {"pro": pro, "error": "This link is invalid or has expired. Please ask the clinic to resend the form.",
               **white_label_context(pro)}
    ...
    if request.method == "POST":
        ...
        if last10_digits(entered) == expected_last10:
            # mark browser session verified for this professional
            request.session[f"phone_verified_{code}"] = True
            return redirect(reverse("content:parent_language_select", args=[code]))
        else:
            error = "The number you entered does not match. Please try again."
...
def screening_form(request, code, lang):
    ...
    if request.method == "POST":
        ...
        if missing:
            ctx = {"error": "Please fill all required fields.", "fields": fields, "lang": lang, "pro": pro, **white_label_context(pro)}
        ...
        # Prepare doctor education links (only for email & doctor page)
        rf_i18n = RedFlagI18n.objects.filter(red_flag_id__in=flags, lang_id=lang)
        rf_labels = [x.parent_label for x in rf_i18n]
        rf_slugs = list(RedFlag.objects.filter(red_flag_code__in=flags).values_list("education_url_slug", flat=True))
        education_links = [request.build_absolute_uri(reverse("content:education_page", args=[slug])) for slug in rf_slugs]

        # Email doctor if flags present
        if flags_count > 0:
            _send_doctor_report_email(
                submission, pro, lang, rf_labels, education_links, patient_name, parent_phone, request
            )

        # Email patient their own report (ONLY patient PDF), regardless of flags
        ...
        # Build patient CTAs (page 14) – only show when flags exist
        ...
        ctx = {
            ...
            "rf_labels": rf_labels,
            # NOTE: education links are intentionally NOT sent to patient page
            "call_link": call_link,
            "wa_link": wa_link,
            "pro": pro,
            **white_label_context(pro),
    ...
    ctx = {"fields": fields, "lang": lang, "pro": pro, **white_label_context(pro)}
    return render(request, "content/screening_form.html", ctx)
...
def _send_doctor_report_email(submission, pro, lang, rf_labels, education_links, patient_name, parent_phone, request):
    ...
    rf_list_html = "<ul>" + "".join(
        f"<li>{label} — <a href='{link}' target='_blank' rel='noopener'>Doctor Education</a></li>"
    ...
    doctor_pdf_bytes, doctor_pdf_pwd = build_doctor_report_pdf_bytes(
        ...
        doctor_first_name=(pro.first_name or ""),   # <-- pass FIRST NAME explicitly
...
def view_result(request, report_code):
    ...
    rf_ids = list(SubmissionRedFlag.objects.filter(submission=sub).values_list("red_flag_id", flat=True))
    rf_labels = list(
        RedFlagI18n.objects.filter(red_flag_id__in=rf_ids, lang_id=sub.lang_id).values_list("parent_label", flat=True)
    )
    rf_slugs = list(RedFlag.objects.filter(red_flag_code__in=rf_ids).values_list("education_url_slug", flat=True))
    education_links = [request.build_absolute_uri(reverse("content:education_page", args=[slug])) for slug in rf_slugs]
