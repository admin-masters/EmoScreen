...
def clinic_send(request, code):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            parent_link = request.build_absolute_uri(reverse("content:parent_language_select", args=[code]))
            msg = parent_message(lang, parent_link)
...
def education_page(request, slug):
    ...
    de = get_object_or_404(DoctorEducation, red_flag=rf, lang_id="en")  # adjust if multilingual doctor education added
    return render(request, "content/education_page.html", {"rf": rf, "de": de})
